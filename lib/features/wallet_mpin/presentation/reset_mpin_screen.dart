...
class ResetMpinScreen extends ConsumerStatefulWidget {
  const ResetMpinScreen({super.key});

  @override
  ConsumerState<ResetMpinScreen> createState() => _ResetMpinScreenState();
}

class _ResetMpinScreenState extends ConsumerState<ResetMpinScreen> {
  ...
  void _submit() async {
    ...
    final success = await ref.read(walletMpinProvider.notifier).resetMpin(_newPinController.text);

    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Wallet MPIN reset successfully!')),
      );
      // Return to security screen or home
      context.pop(); // pops Reset
      if (context.canPop()) context.pop(); // pops Forgot
    }
  }
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              const Text('Enter New MPIN', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: state.isLoading
                      ...
                      : const Text('Reset MPIN', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                ),
              ),
            ],
          ),
