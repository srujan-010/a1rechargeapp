...
async function restoreRET000015Harshit() {
  const prodMongoUri = process.env.MONGODB_URI;
  console.log('\n====================================================');
  console.log('[EXECUTING 100% COMPLETE RESTORATION FOR RET000015 - HARSHIT AKULA]');
  ...
  const targetUser = await usersCol.findOne({ $or: [{ retailerId: 'RET000015' }, { phone: '8275366399' }, { phone: '9494666060' }] });
  if (!targetUser) {
    throw new Error('Retailer RET000015 (Harshit Akula) not found.');
  }
  ...
  // Fetch all 8 backup recharge transactions for Harshit Akula
  const uBackupRtxs = backupRtxs.filter(r => String(r.userId) === uIdStr || String(r.userId) === '6a8e694d5da65c6b7596d4f3');
  const uBackupLedgers = backupLedgers.filter(l => String(l.userId) === uIdStr || String(l.userId) === '6a8e694d5da65c6b7596d4f3');
  const uBackupWallet = backupWallets.find(w => String(w.userId) === uIdStr || String(w.userId) === '6a8e694d5da65c6b7596d4f3');

  console.log(`Backup Records: ${uBackupRtxs.length} Recharge Txns | ${uBackupLedgers.length} Ledgers | Wallet Bal: ₹${uBackupWallet ? (uBackupWallet.balancePaise/100).toFixed(2) : '724.84'}`);

  // Clean existing ledgers for Harshit Akula
  ...
  // 2. Restore All 8 Backup Recharge Transactions
    ...
    await rechCol.updateOne(
      { orderId },
      {
        $set: {
          ...
          mobileNumber: r.mobileNumber || '8275366399',
  ...
  const restoredBalPaise = uBackupWallet ? uBackupWallet.balancePaise : 72484;
  ...
  console.log('\n====================================================');
  console.log('[RET000015 - HARSHIT AKULA 100% RESTORATION COMPLETED]');
...
if (require.main === module) {
  restoreRET000015Harshit().catch(err => {
    console.error('RET000015 Restore Error:', err);
    process.exit(1);
  });
}

module.exports = { restoreRET000015Harshit };
