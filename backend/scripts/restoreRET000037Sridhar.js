...
async function restoreRET000037Sridhar() {
  const prodMongoUri = process.env.MONGODB_URI;
  console.log('\n====================================================');
  console.log('[EXECUTING 100% COMPLETE RESTORATION FOR RET000037 - SRIDHAR]');
  ...
  const targetUser = await usersCol.findOne({ $or: [{ retailerId: 'RET000037' }, { phone: '9404979104' }] });
  if (!targetUser) {
    throw new Error('Retailer RET000037 (sridhar) not found.');
  }

  const uId = targetUser._id;
  console.log(`Retailer Found: ${targetUser.name} (${targetUser.phone}) | ID: ${uId}`);

  // Clean old ledgers for RET000037
  await ledgersCol.deleteMany({ userId: uId });

  // 1. Add Money Credit: ₹250.00
  const addMoneyLedger = {
    ...
    referenceId: 'RET37_ADD_250',
    description: '₹250.00 has been added to your A1 Recharge wallet.',
    remark: 'WALLET_TOPUP',
    createdAt: new Date('2026-09-01T13:43:00.000Z'),
    updatedAt: new Date('2026-09-01T13:43:00.000Z'),
  };
  await ledgersCol.insertOne(addMoneyLedger);
  console.log('  => Add Money Credit Restored: +₹250.00 (25000 paise)');

  // 2. Real Recharge Transactions
  const rechTxns = [
    {
      orderId: 'A1R1788092639824555',
      ...
      netPaise: 1000,
      status: 'FAILED',
      date: new Date('2026-08-30T12:54:11.138Z'),
    },
    {
      orderId: 'A1R1788269967998664',
      mobileNumber: '9404979104',
      op: 'TATA SKY',
      grossPaise: 25000,
      commPaise: 750, // 3.0% = ₹7.50
      netPaise: 24250, // ₹242.50
      status: 'SUCCESS',
      date: new Date('2026-09-01T13:44:13.438Z'),
    },
  ];

  let currentBalPaise = 25000;
        ...
        await commCol.updateOne(
          { transactionId: rTxDoc._id },
          {
            $set: {
              ...
              providerCommissionAmountPaise: 1000, // 4.0%
              retailerCommissionAmountPaise: 750, // 3.0%
              companyProfitAmountPaise: 250, // 1.0%
              rechargeAmount: grossAmt,
              providerCommissionPercentage: 4.0,
              providerCommissionAmount: 10.0,
              retailerCommissionPercentage: 3.0,
              retailerCommissionAmount: 7.50,
              companyProfitPercentage: 1.0,
              companyProfitAmount: 2.50,
              createdAt: r.date,
              updatedAt: r.date,
            }
          },
  ...
  console.log('\n====================================================');
  console.log('[RET000037 - SRIDHAR 100% RESTORATION COMPLETED]');
  console.log(`Add Money Credit       : +₹250.00`);
  console.log(`Recharge Transactions  : 2 Total (1 Tata Sky ₹250.00 SUCCESS | 1 FAILED)`);
...
if (require.main === module) {
  restoreRET000037Sridhar().catch(err => {
    console.error('RET000037 Restore Error:', err);
    process.exit(1);
  });
}

module.exports = { restoreRET000037Sridhar };
